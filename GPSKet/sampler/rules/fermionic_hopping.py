...
def transition_function(
    key, sample, hop_probability, transition_probs=None, return_updates=False
):
    def apply_electron_hop(samp, key):
        ...
        hopping_or_exchange = jax.random.choice(
            keyC, jnp.array([0, 1]), p=jnp.array([hop_probability, 1 - hop_probability])
        )
        occ_prob = jnp.where(
            hopping_or_exchange == 0, occ, jnp.logical_and(occ != 2, occ != 0)
        )
        ...
        target_site_probs = jnp.where(
            ...
            jnp.logical_and(
                jnp.logical_and(occ != 2, occ != 0), ~((samp & spin).astype(bool))
            ),
        )
        if transition_probs is not None:
            target_site = jax.random.choice(
                keyD, samp.shape[-1], p=transition_probs[start_site, :]
            )
        else:
            target_site = jax.random.choice(keyD, samp.shape[-1], p=target_site_probs)
        # Make sure no unallowed move is applied
        target_site = jnp.where(
            target_site_probs[target_site] == False, start_site, target_site
        )
        ...
        return (
            jax.lax.cond(
                hopping_or_exchange == 0, lambda _: updated_sample, get_exchange, None
            ),
            start_site,
            target_site,
        )
    ...
    updated_sample, start_sites, target_sites = jax.vmap(
        apply_electron_hop, in_axes=(0, 0), out_axes=(0, 0, 0)
    )(sample, keys)
...
transition_fun_with_update = (
    lambda key, sample, hop_probability, transition_probs: transition_function(
        key, sample, hop_probability, transition_probs, return_updates=True
    )
)
transition_fun_without_update = (
    lambda key, sample, hop_probability, transition_probs: transition_function(
        key, sample, hop_probability, transition_probs, return_updates=False
    )
)
