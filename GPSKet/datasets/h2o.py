...
def _H2O_raw(basis, method, datapath):
    ...
    if not os.path.exists(os.path.join(datapath, f"all_configs_H2O_{dsid}.npy")):
        ...
        def to_config(alpha, beta, n):
            string_a = [0] * (n - (len(bin(alpha)) - 2)) + [
                1 if digit == "1" else 0 for digit in bin(alpha)[2:]
            ]
            string_a.reverse()
            string_b = [0] * (n - (len(bin(beta)) - 2)) + [
                1 if digit == "1" else 0 for digit in bin(beta)[2:]
            ]
            string_b.reverse()
            return np.array(string_a, dtype=np.uint8) + 2 * np.array(
                string_b, dtype=np.uint8
            )
        ...
        np.save(
            os.path.join(datapath, f"all_configs_H2O_{dsid}.npy"), all_configurations
        )
        np.save(
            os.path.join(datapath, f"all_amplitudes_H2O_{dsid}.npy"), all_amplitudes
        )
    else:
        all_configurations = np.load(
            os.path.join(datapath, f"all_configs_H2O_{dsid}.npy")
        )
        all_amplitudes = np.load(
            os.path.join(datapath, f"all_amplitudes_H2O_{dsid}.npy")
        )
...
def get_H2O_dataset(
    basis: str = "CANONICAL",
    method: str = "FCI",
    select_largest: Optional[int] = None,
    datapath: str = _DATA,
) -> Tuple[Array, Array]:
    """
    ...
    """
    configurations, amplitudes = _H2O_raw(
        basis.upper(), method.upper(), datapath=datapath
    )
    if select_largest:
