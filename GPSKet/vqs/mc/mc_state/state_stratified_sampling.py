...
class MCStateStratifiedSampling(MCStateUniqueSamples):
    def __init__(
        ...
        **kwargs
        ...
        self.deterministic_samples = jnp.array_split(deterministic_samples, _n_nodes)[
            _rank
        ]

        self.N_complement = (
            N_total - deterministic_samples.shape[0]
        )  # Total size of the complement

        self.rand_norm = rand_norm

        self.lookup_dict = {
            tuple(conf): i for i, conf in enumerate(np.array(deterministic_samples))
        }
        ...
        for j in range(self.sampler.n_chains_per_rank):
            samp = self.sampler.hilbert.random_state(
                key, dtype=self.deterministic_samples.dtype
            ).reshape(-1)
        ...
        if number_random_samples is None:
            ...
            self.number_random_samples = len(
                np.array_split(np.arange(number_random_samples), _n_nodes)[_rank]
            )
    ...
    def sample_step(self):
        ...
        self.current_sample = self.sample(
            chain_length=1, n_discard_per_chain=0
        ).reshape(self.current_sample.shape)
        ...
        self.current_sample = jnp.where(
            np.expand_dims(valid, -1), self.current_sample, old_sample
        )
    ...
    def samples_with_counts(self) -> Tuple[jnp.ndarray, jnp.ndarray]:
        if self._unique_samples is None:
            ...
            samples_from_complement = jnp.array(np.array(sampled_configs)).reshape(
                (-1, self.current_sample.shape[-1])
            )

            all_samples = jnp.concatenate(
                (self.deterministic_samples, samples_from_complement)
            )

            def log_prob(samp):
                return jnp.squeeze(2 * self.log_value(samp.reshape((1, -1))).real)

            log_prob_amps_deterministic = nkjax.vmap_chunked(
                log_prob, chunk_size=self.chunk_size
            )(self.deterministic_samples)
            log_prob_amps_complement = nkjax.vmap_chunked(
                log_prob, chunk_size=self.chunk_size
            )(samples_from_complement)

            total_samples = _sum(len(log_prob_amps_complement))
            if (
                self.renormalize
            ):  # Switch off for normalized models (e.g. autoregressive models)
                # Renormalise the probability amplitudes for numerical stability
                rescale_shift = _mpi_max_jax(
                    jnp.max(
                        jnp.concatenate(
                            (log_prob_amps_deterministic, log_prob_amps_complement)
                        )
                    )
                )[0]
                ...
                if self.rand_norm:
                    ...
                    key = jax.random.split(self.sampler_state.rng)[0]
                    random_samples = np.empty(
                        (
                            self.number_random_samples,
                            self.deterministic_samples.shape[-1],
                        ),
                        dtype=self.deterministic_samples.dtype,
                    )
                    found_samples = 0
                    while found_samples < self.number_random_samples:
                        key = jax.random.split(key)[0]
                        proposed_samples = np.array(
                            self.sampler.hilbert.random_state(
                                key,
                                size=self.number_random_samples,
                                dtype=self.deterministic_samples.dtype,
                            )
                        )
                    ...
                    log_probs_sampled = nkjax.vmap_chunked(
                        log_prob, chunk_size=self.chunk_size
                    )(random_samps)

                    norm_sampled = (
                        self.N_complement
                        * _sum(jnp.exp(log_probs_sampled - rescale_shift))
                        / _sum(random_samps.shape[0])
                    )
                    ...
                    norm_sampled = (
                        self.N_complement
                        * total_samples
                        / _sum(jnp.exp(-log_prob_amps_complement))
                    )
            ...
            prefactors_sampled = (
                jnp.ones(log_prob_amps_complement.shape)
                * (1 - norm_deterministic / norm_estimate)
                / total_samples
            )

            self._unique_samples = all_samples
            self._relative_counts = jnp.concatenate(
                (prefactors_det, prefactors_sampled)
            )
