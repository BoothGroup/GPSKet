...
class PlaquetteqGPS(nn.Module):
    ...
    apply_fast_update: bool = (
        False  # Careful with zero-valued parameters here, TODO: implement fallback...
    )
    ...
    def setup(self):
        if type(self.syms) == tuple:
            ...
            if self.apply_fast_update:
                warnings.warn(
                    "Attention! Fast updating is not applied in qGPS as the inverse symmetry operations are not supplied."
                )
    ...
    def __call__(self, inputs, cache_intermediates=False, update_sites=None):
        ...
        epsilon = self.param(
            ...
            (
                self.local_dim,
                self.plaquettes.shape[0],
                self.M,
                self.plaquettes.shape[1],
            ),
            self.dtype,
        )
        ...
        if cache_intermediates or (update_sites is not None):
            site_product_save = self.variable(
                "intermediates_cache",
                "site_prod",
                lambda: jnp.zeros(0, dtype=self.dtype),
            )
            indices_save = self.variable(
                "intermediates_cache",
                "samples",
                lambda: jnp.zeros(0, dtype=indices.dtype),
            )
        ...
        if update_sites is None:
            ...
            def evaluate_site_product(sample):
                def single_plaquette_eval(plaquette_ids, epsi):
                    result = jnp.take_along_axis(
                        epsi, sample[:, :, plaquette_ids], axis=0
                    ).prod(axis=-1)
                    return result.reshape(-1)

                return jax.vmap(single_plaquette_eval, in_axes=(0, 1), out_axes=0)(
                    plaquettes, epsilon
                )

            def get_site_prod(sample):
                return jax.vmap(evaluate_site_product, in_axes=-1, out_axes=-1)(
                    self.symmetries(sample)
                )

            transformed_samples = jnp.expand_dims(
                indices, (1, 2)
            )  # required for the inner take_along_axis
            site_product = jax.vmap(get_site_prod)(transformed_samples)
        else:
            site_product_old = site_product_save.value
            new_samples = indices
            old_samples = jax.vmap(jnp.take, in_axes=(0, 0), out_axes=0)(
                indices_save.value, update_sites
            )
            ...
            def inner_site_product_update(site_prod_old, new_occs, old_occs, sites):
                def single_plaquette_update(sp_old, epsi, inv_plaquette):
                    ...
                    update = jnp.where(
                        jnp.expand_dims(valid_elements, 1),
                        x=epsi[new_occs, :, transformed_sites],
                        y=1.0,
                    ).prod(axis=0)
                    update /= jnp.where(
                        jnp.expand_dims(valid_elements, 1),
                        x=epsi[old_occs, :, transformed_sites],
                        y=1.0,
                    ).prod(axis=0)
                    return sp_old * update

                return jax.vmap(single_plaquette_update, in_axes=(0, 1, 0), out_axes=0)(
                    site_prod_old, epsilon, inv_plaquette_ids
                )

            def outer_site_product_update(
                site_prod_old, sample_new, sample_old, update_sites
            ):
                inv_sym_new, inv_sym_sites = self.symmetries_inverse(
                    sample_new, update_sites
                )
                inv_sym_old, inv_sym_sites = self.symmetries_inverse(
                    sample_old, update_sites
                )

                return jax.vmap(
                    inner_site_product_update, in_axes=(-1, -1, -1, -1), out_axes=-1
                )(site_prod_old, inv_sym_new, inv_sym_old, inv_sym_sites)

            site_product = jax.vmap(
                outer_site_product_update, in_axes=(0, 0, 0, 0), out_axes=0
            )(site_product_old, new_samples, old_samples, update_sites)
        ...
        if cache_intermediates:
            ...
            if update_sites is not None:
                ...
                indices_save.value = jax.vmap(
                    update_fun, in_axes=(0, 0, 0), out_axes=0
                )(indices_save.value, update_sites, indices)
            else:
