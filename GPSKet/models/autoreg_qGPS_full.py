...
class ARqGPSFull(AbstractARqGPS):
    ...
    M: Union[
        int, HashableArray
    ]  # If M is a list, it defines a per-site support dimension -> this should be faster to evaluate but gives a significant compilation overhead
    """Bond dimension"""
    ...
    init_fun: Optional[
        NNInitFunc
    ] = None  # Defaults to qGPS-normal with the parameter dtype
        ...
        if isinstance(self.M, HashableArray):
            self._epsilon = tuple(
                [
                    self.param(
                        "epsilon_{}".format(i),
                        init,
                        (self.hilbert.local_size, np.asarray(self.M)[i], i + 1),
                        self.dtype,
                    )
            ...
            self._epsilon = self.param(
                ...
                (
                    ...
                    self.M,
                    int(self.hilbert.size * (self.hilbert.size + 1) / 2),
                ),
                self.dtype,
            )
        if self.apply_fast_update:
            self._saved_configs = self.variable(
                "intermediates_cache", "samples", lambda: None
            )
            if isinstance(self.M, HashableArray):
                self._saved_context_product = tuple(
                    [
                        self.variable(
                            "intermediates_cache",
                            "context_prod_{}".format(i),
                            lambda: None,
                        )
                        for i in range(self.M.shape[0])
                    ]
                ...
                self._saved_context_product = self.variable(
                    "intermediates_cache", "context_prod", lambda: None
                )
    ...
    def __call__(
        self, inputs: Array, cache_intermediates=False, update_sites=None
    ) -> Array:
        ...
        if update_sites is not None:
            ...
            prev_occupancies = jax.vmap(jnp.take, in_axes=(0, 0), out_axes=0)(
                saved_input, update_sites
            )
            ...
            # Old occupancy
            old_occupancy, site_indices = self.apply_symmetries[1](
                prev_occupancies, update_sites
            )  # (B, #updates, T), (B, #updates, T)

            # Updated occupancy
            new_occupancy, site_indices = self.apply_symmetries[1](
                inputs, update_sites
            )  # (B, #updates, T), (B, #updates, T)

            update_args = (
                self.hilbert.states_to_local_indices(old_occupancy),
                self.hilbert.states_to_local_indices(new_occupancy),
                site_indices,
            )
            ...
            full_samples = jax.vmap(update_fun, in_axes=(0, 0, 0), out_axes=0)(
                self._saved_configs.value, update_sites, inputs
            )
        else:
            update_args = None
            full_samples = inputs
        ...
        full_samples_sym = self.hilbert.states_to_local_indices(
            full_samples_sym
        )  # (B, L, T)
        ...
        if update_sites is not None:
            if isinstance(self._epsilon, tuple):
                old_contexts = [
                    cont_prod.value for cont_prod in self._saved_context_product
                ]
            else:
                old_contexts = self._saved_context_product.value

            log_psi, context_products = jax.vmap(
                _conditionals, in_axes=(None, -1, -1, -1), out_axes=(-1, -1)
            )(
                self, full_samples_sym, update_args, old_contexts
            )  # (B, L, D, T), (L, B, M, T)
        else:
            log_psi, context_products = jax.vmap(
                _conditionals, in_axes=(None, -1, None, None), out_axes=(-1, -1)
            )(
                self, full_samples_sym, None, None
            )  # (B, L, D, T), (L, B, M, T)
        if self.normalize:
...
def _compute_conditional(
    ...
    saved_context_prod: Optional[Array] = None,
    ...
    if update_args is None:
        if isinstance(model._epsilon, tuple):
            ...
            local_epsilon = jax.lax.dynamic_slice_in_dim(
                model._epsilon, lower_index, model.hilbert.size, axis=-1
            )
        ...
        context_val = jnp.take_along_axis(
            context_param, inputs_expanded, axis=1
        ).reshape(
            (-1, *local_epsilon.shape[-2:])
        )  # (B, M, L)

        # Apply masking for sites > index
        context_val = jnp.where(
            jnp.arange(local_epsilon.shape[-1]) >= index, 1.0, context_val
        )
        ...
        # Apply masking for sites > index
        update = jnp.where(jnp.expand_dims(site_indices >= index, axis=-1), 1.0, update)
    ...
    log_psi = gpu_cond(
        ...
        + model.renormalize_log_psi(n_spins, model.hilbert, index),
        lambda log_psi: log_psi,
        log_psi,
    )
...
def _conditionals(
    model: ARqGPSFull,
    inputs: Array,
    update_args: Optional[Tuple[Tuple[Array, Array], Tuple[Array, Array]]] = None,
    saved_context_product: Optional[Array] = None,
) -> Array:
    ...
    def _scan_fun(n_spins, index):
        if saved_context_product is not None:
            if isinstance(model._epsilon, tuple):
                n_spins, log_psi, context_product = _compute_conditional(
                    model,
                    n_spins,
                    inputs,
                    index,
                    update_args,
                    saved_context_product[index][:, :],
                )
            else:
                n_spins, log_psi, context_product = _compute_conditional(
                    model,
    ...
    if isinstance(model._epsilon, tuple):
        ...
        _, value = jax.lax.scan(_scan_fun, n_spins, indices)
...
class ARqGPSModPhaseFull(ARqGPSFull):
    ...
    def setup(self):
        ...
        super().setup()
        self._qgps = qGPS(
            self.hilbert, self.hilbert.size, dtype=jnp.float64, init_fun=self.init_fun
        )
