...
class FermiHubbard(FermionicDiscreteOperator):
    ...
    def get_conn_flattened(self, x, sections, pad=True):
        x_primes, mels = self._get_conn_flattened_kernel(
            np.asarray(x, dtype=np.uint8), sections, self.U, self.edges, self.t
        )
    ...
    def _get_conn_flattened_kernel(x, sections, U, edges, t):
        ...
        for batch_id in range(x.shape[0]):
            ...
            for edge_count in range(edges.shape[0]):
                ...
                # spin up
                x_prime[count, :] = x[batch_id, :]
                mels[count] = -t[edge_count] * apply_hopping(
                    edge[0], edge[1], x_prime[count], 1, cummulative_count=up_count
                )
                count += 1
                x_prime[count, :] = x[batch_id, :]
                mels[count] = -t[edge_count] * apply_hopping(
                    edge[1], edge[0], x_prime[count], 1, cummulative_count=up_count
                )
                count += 1
                ...
                x_prime[count, :] = x[batch_id, :]
                mels[count] = -t[edge_count] * apply_hopping(
                    edge[0], edge[1], x_prime[count], 2, cummulative_count=down_count
                )
                count += 1
                x_prime[count, :] = x[batch_id, :]
                mels[count] = -t[edge_count] * apply_hopping(
                    edge[1], edge[0], x_prime[count], 2, cummulative_count=down_count
                )
                count += 1
...
def local_en_on_the_fly(
    logpsi, pars, samples, args, use_fast_update=False, chunk_size=None
):
    edges, U, t = args
        ...
        def get_hopping_term(spin_int, cumulative_count):
            def apply_hopping(annihilate_site, create_site):
                ...
                multiplicator = jax.lax.cond(
                    jnp.logical_or(
                        ~(start_occ & spin_int).astype(bool),
                        (end_occ & spin_int).astype(bool),
                    ),
                    no_hop,
                    hop,
...
def get_local_kernel(
    vstate: nk.vqs.MCState, op: FermiHubbardOnTheFly, chunk_size: Optional[int] = None
):
    try:
    ...
    return nkjax.HashablePartial(
        local_en_on_the_fly, use_fast_update=use_fast_update, chunk_size=chunk_size
    )
