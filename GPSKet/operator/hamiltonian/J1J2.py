...
def local_en_on_the_fly(
    ...
    def vmap_fun(sample):
        ...
        def inner_vmap(operator_element, acting_on_element):
            ...
            off_diag_connected = jnp.array(
                [0, 2, 1, 3]
            )  # indices of the non-zero off-diagonal element (or the diagonal index if no non-zero off-diagonal exists)

            def compute_element(connected_index):
                mel = operator_element[basis_index, connected_index]
                new_occ = (
                    2 * jnp.array([connected_index % 2, connected_index // 2]) - 1.0
                )  # map back to standard netket representation of spin configurations
                if use_fast_update:
                    log_amp_connected = logpsi(
                        parameters,
                        jnp.expand_dims(new_occ, 0),
                        update_sites=jnp.expand_dims(acting_on_element, 0),
                    )
                else:
            ...
            off_diag = jnp.where(
                off_diag_connected[basis_index] != basis_index,
                compute_element(off_diag_connected[basis_index]),
                0.0,
            )
...
def get_local_kernel(
    vstate: nk.vqs.MCState, op: HeisenbergOnTheFly, chunk_size: Optional[int] = None
):
    try:
    ...
    return nkjax.HashablePartial(
        local_en_on_the_fly,
        op.hilbert.states_to_local_indices,
