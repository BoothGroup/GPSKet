...
arqgps = ARPlaquetteqGPS(hi, M, plaquettes, masks, dtype=dtype)
...
arqgps_symm = ARPlaquetteqGPS(
    hi, M, plaquettes, masks, dtype=dtype, apply_symmetries=apply_symmetries
)
...
for t in tqdm(range(n_symm), desc="Test #2"):
    inputs_t = jnp.take_along_axis(
        inputs, jnp.tile(symmetries[:, t], (batch_size, 1)), 1
    )
...
for l in tqdm(range(L), desc="Test #3.1, unconstrained"):
    p, variables = arqgps_symm.apply(
        variables,
        inputs,
        (l, _masks[l], _plaquettes[l]),
        method=ARPlaquetteqGPS._conditional,
        mutable=True,
    )
