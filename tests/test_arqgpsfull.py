...
arqgps = ARqGPSFull(hi, M, dtype=dtype)
...
arqgps_symm = ARqGPSFull(hi, M, dtype=dtype, apply_symmetries=apply_symmetries)
...
for l in tqdm(range(L), desc="Test #3.1, unconstrained"):
    p, variables = arqgps_symm.apply(
        variables, inputs, l, method=ARqGPSFull._conditional, mutable=True
    )
