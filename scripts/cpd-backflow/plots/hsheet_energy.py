...
n_atoms = 36
rhf_energies = pd.read_csv('hsheet_rhf.csv')
uccsd_energies = pd.read_csv('hsheet_uccsd.csv')
dmrg_energies = pd.read_csv('hsheet_dmrg.csv')
gpsxsd_energies = pd.read_csv('hsheet_gpsxsd.csv')
...
sub_df = df.query('`distance` != 2.5 and `exchange_cutoff`.isna() and `M` == 1', engine="python")
label = r"$\Psi^{CPD} (M=1)$"
...
sub_df = df.query('`distance` != 2.5 and `exchange_cutoff` == 5.0 and `M` == 1')
label = r"$\Psi^{CPD} (M=1, K=5)$"
error_wrt_dmrg = (sub_df['energy']-dmrg_energies_values)/n_atoms
axs[1].plot(sub_df['distance'], error_wrt_dmrg, color="0.2", linestyle="--", label=label)
