...
def read_config(workdir):
    if os.path.isdir(workdir):
        ...
        raise ValueError(
            f"The argument {workdir} is not a valid path to an existing directory."
        )
...
def restore_best_params(workdir: str, filename: str='best_params'):
    if not os.path.isdir(workdir):
        raise ValueError(f"The argument {workdir} is not a valid path to an existing directory.")
    params_file = os.path.join(workdir, f"{filename}.mpack")
    if os.path.isfile(params_file):
        with open(params_file, "rb") as b:
            best_params = serialization.msgpack_restore(b.read())
    else:
        best_params = None
    return best_params

def save_best_params(workdir: str, params: PyTree, filename: str='best_params'):
    if not os.path.isdir(workdir):
        raise ValueError(f"The argument {workdir} is not a valid path to an existing directory.")
    params_file = os.path.join(workdir, f"{filename}.mpack")
    with open(params_file, "wb") as f:
        binary_data = serialization.to_bytes(params)
        f.write(binary_data)