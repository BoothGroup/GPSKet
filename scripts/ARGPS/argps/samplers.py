...
def get_sampler(
    ...
    if hasattr(config.model, "normalize"):
        if not config.model.normalize and config.sampler_name == "ARDirectSampler":
            raise ValueError(
                "The ARDirectSampler can only be used with normalized autoregressive models"
            )
    try:
        sa_cls = _SAMPLERS[config.get("sampler_name")]
    except KeyError:
        raise ValueError(
            f"Sampler {config.sampler_name} is not a valid class or is not supported yet."
        )
    kwargs = config.to_dict()["sampler"]
    if (
        config.system.get("molecule", None)
        and config.sampler_name != "MetropolisHopping"
    ):
